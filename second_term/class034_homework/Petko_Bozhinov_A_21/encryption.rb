...
if ARGV[0] == "generate"
  if ARGV.include? "pubkey"
    puts "Public key    : %x " % n
  end
end

if ARGV[0] == "operate"
  hash = Hash[ARGV.map.with_index.to_a]
  if ARGV.include? "message"
    m = ARGV[hash["message"]+1] 
    if ARGV.include? "pubkey"
      n = ARGV[hash["pubkey"]+1]
      n = n.to_i(16)
    end
    c = RSA.encrypt m, n
    p "Encrypted message: %x" % c
    if not ARGV.include? "pubkey"
      zr= RSA.decrypt c, n, d if not ARGV.include? "pubkey"
      p "Decrypted message: %s" % zr
    end
  end
end