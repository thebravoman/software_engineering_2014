...
#			if extension == "rb"
#				results[script_file1][folder+6] = `flog #{repository}`.to_s.split(" ").first.to_s.split(":").first	
#				results[script_file1][folder+12] = `flay #{repository}`.to_s.split("= ").last.to_s.split(")").first.to_i
#			end
		end
	end
	folder += 1
