...
Dir.glob(ARGV[0] + "*").each do |file1|
    ...
    if (ext1 != "rb") or (short1.scan("_").count != 2)
...
Dir.glob(ARGV[1] + "*").each do |file2|
    ...
    if (ext2 != "rb") or (short2.scan("_").count != 2)
...
# undefined local variable `digit'
*Unfixed*
...
# the program outputs and correct files that are in the format FirsrName_LastName_digits.rb (by condition "Find all the files from both folders that are not in the format FirsrName_LastName_digits.rb")
*Unfixed*
