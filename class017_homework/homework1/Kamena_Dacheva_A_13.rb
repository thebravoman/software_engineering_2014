# Run: ruby Kamena_Dacheva_A_13.rb /home/kamena/Desktop/software_engineering_2014/class017_test/files_for_exam_2/results/ /home/kamena/Desktop/software_engineering_2014/class017_test/files_for_exam_2/expects/

require './Kamena_Dacheva_html_writer.rb'
...
def argv script_file, dir
	script_file.reverse.split("/", 5).last.reverse + "#{dir}"
end

def check_for_writer prog_name
	prog_name.include?("writer") || prog_name.include?("Writer")
end
...
Dir.glob(ARGV[0]+"*_*_*_*.rb").each do |script_file|
	...
	names = prog_name.reverse.split("_", 3).last.reverse.gsub("_"," ")
	unless check_for_writer prog_name
		results[names] = 1
	end
	Dir.glob(ARGV[1]+"*").each do |fixture|
		expected = fixture.split(".").first.split("/").last 
		unless ((check_for_writer prog_name) || (program != expected))
			if stefan.include?"#{expected}" 
				argv = argv script_file, "/class016/Stefan_Iliev_B_28/fixture"
			elsif vanya.include?"#{expected}" 
				argv = argv script_file, "/class016/Vanya_Santeva_5_B/fixture/"
			elsif rado.include?"#{expected}" 
				argv = argv script_file, "/class016/Radoslav_Kostadinov_22_A/fixture"
			elsif moreti.include?"#{expected}" 
				argv = argv script_file, "/class014_homework/"
			end
