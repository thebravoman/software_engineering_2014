...
def zero_level_checking(short_name, program_name, names)
		names[short_name][0] = 0 unless program_name =~ /\/[a-zA-Z0-9]+_[a-zA-Z0-9]+_[0-9]+\.[A-Za-z]+\z/ 
...
CSV.open("results_Dimitar_Matev_A_5.csv", "w") do |csv|
	csv << [" ", " ", "VH", "002", "003", "004", "009", "012"]
	names.sort_by{ |key, value| key }.each do |key, value| 	
		csv << key.split(" ") + value.map { |var| var.nil? ? 0 : var }.map { |var| var.class == Array ? var[0] : var }
	end
end
