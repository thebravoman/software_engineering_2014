...
#require_relative "svg_writer_OP.rb"
...
CSV.foreach(ARGV[0]+"/class009_homework/project_to_names.csv") do |row|
	...
	if row.first != "Project Name" && row.last != nil
...
while array_count < students_list.length
	if mapping.include?(students_list[array_count])
...
while array_count < students_list.length
	if mapping.include?(students_list[array_count])
		if mapping_keys[array_count] != "Lazy Slobs" && mapping_keys[array_count] != "require\'teamName\'" && mapping_keys[array_count] != "YellowPatka" && mapping_keys[array_count] != "Happy_Satan" && mapping_keys[array_count] != "The_inventors" && mapping_keys[array_count] != nil
...
if ARGV[1] == "-o"
	...
	writer.write students_list, vhodno_nivo_res, homework2_res, homework3_res, homework4_res, mapping_res, homework12_res
end
