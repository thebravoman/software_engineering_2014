...
Dir.glob("#{ARGV[0]}*_*_*.rb") do |file1|
    filename = file1.split('/').last.split('.').first
    if (filename.split("_").size == 3)
	Dir.glob("#{ARGV[1]}*_*_*.rb") do |file2|
	    filename = file2.split('/').last.split('.').first
	    if (filename.split("_").size == 3)
		...
		hash1[lastName1] = firstName1 + "."
		hash2[lastName2] = firstName2 + "."
	    end
	end
    end
end    
