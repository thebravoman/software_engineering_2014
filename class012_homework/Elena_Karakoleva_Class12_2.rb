...
Dir.glob(ARGV[0]+"*2.rb").each do |script_file|
	...
	script_file1 = script_file.split("/").last.reverse.split("_", 3).last.reverse.gsub("_"," ")
	if !git_log.empty?
		results[script_file1] = Hash.new
		count += 1
				...
				if File.exists?(expected_file) 
					...
					file_content = File.read(expected_file)
						...
						if (output <=> file_content) == 0
							results[script_file1][fixture_value] = 1
						else
							results[script_file1][fixture_value] = 0
						end
