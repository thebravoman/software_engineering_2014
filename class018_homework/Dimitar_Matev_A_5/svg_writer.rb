...
class SVGWriter
	...
	def write name, name_of_folder, folder_names
		...
		(folder_names.size-2).times do |index|
			...
			case folder_names[index_curr]
			when /g\d+/ then sub = 180
			when /y\d+/ then sub = 140	
			else sub = 1
			end
			x = cal_x index
			total_sub = total_result/sub
			result_func = write_rect(x, total_sub, 45, 300-total_sub, sub)
			svgb.add_element(result_func)
			result_func = write_text((folder_names[index_curr].length/2) + x, 320, folder_names[index_curr])
			svgb.add_element(result_func)
			result_func = write_text(x, 280-total_sub, total_result)
			svgb.add_element(result_func)
		end
